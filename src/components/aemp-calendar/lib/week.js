...
class WeekMode {
  ...
  switchWeek (view, day) {
    return new Promise((resolve, reject) => {
      if (this.vm.cConfig.multi) return logger.warn('多选模式不能切换周月视图');
      const { selectedDay, curYear, curMonth } = this.vm.calendar;
      if (!selectedDay.length) return this.__tipsWhenCanNotSwtich();
      const currentDay = selectedDay[0];
      if (view === 'week') {
        if (this.vm.calendar.weekMode) return;
        const selectedDate = day || currentDay;
        const { year, month } = selectedDate;
        if (curYear !== year || curMonth !== month)
          return this.__tipsWhenCanNotSwtich();
        this.vm.calendar.weekMode = true;
        this.jump(selectedDate)
          .then(resolve)
          .catch(reject);
      } else {
        this.vm.calendar.weekMode = false;
        Render(this.vm)
  ...
  updateCurrYearAndMonth (type) {
    let { days, curYear, curMonth } = this.vm.calendar;
   ...
   */
  calculateLastDay () {
    const { days = [], curYear, curMonth } = this.vm.calendar;
  ...
  firstWeekInMonth (year, month, firstDayOfWeekIsMon) {
    ...
    let { days = [] } = this.vm.calendar.days;
    if (this.vm.calendar.weekMode) {
      days = Day(this.vm).buildDate(year, month);
    }
  ...
  initSelectedDay (dates) {
    let datesCopy = [...dates];
    const { selectedDay = [] } = this.vm.calendar;
    ...
    const {
      disableDateTimestamp,
      disableType
    } = this.__getDisableDateTimestamp(this.vm.cConfig);
    datesCopy = datesCopy.map(item => {
      if (!item) return {};
      const dateTimestamp = getDateTimeStamp(item);
      let date = { ...item };
      date.choosed = selectedDayStr.includes(`${+date.year}-${+date.month}-${+date.day}`);
      if (
      ...
      date = this.__setTodoWhenJump(date, this.vm.cConfig);
      if (this.vm.cConfig.showLunar) {
        date = this.__setSolarLunar(date);
      }
      if (this.vm.cConfig.highlightToday) {
  ...
  /**
   * 周视图下设置可选日期范围
   * @param {object} dates 当前展示的日期
   */
  setEnableAreaOnWeekMode (dates = []) {
    let { enableAreaTimestamp = [], enableDaysTimestamp = [] } = this.vm.calendar;
    dates.forEach(item => {
  ...
  calculateNextWeekDays () {
    ...
    let { curYear, curMonth } = this.vm.calendar;
    let days = [];
    ...
    this.vm.calendar.curYear = curYear;
    this.vm.calendar.curMonth = curMonth;
    this.vm.calendar.days = days;
    this.vm.$nextTick(() => {
      Day(this.vm).setDateStyle();
    })
  }
  ...
  jump ({ year, month, day }) {
    return new Promise(resolve => {
      if (!day) return;
      const firstDayOfWeekIsMon = this.vm.cConfig.firstDayOfWeek === 'Mon';
      ...
      this.vm.calendar.curYear = year;
      this.vm.calendar.curMonth = month;
      this.vm.calendar.empytGrids = [];
      this.vm.calendar.lastEmptyGrids = [];
      this.vm.calendar.selectedDay = dates.filter(item => item.choosed);
      this.vm.$nextTick(() => {
        Day(this.vm).setDateStyle();
  ...
  __setTodoWhenJump (dateInfo) {
    const date = { ...dateInfo };
    const { todoLabels = [], showLabelAlways } = this.vm.calendar;
  ...
  __calculateDates ({ year, month, day }, firstDayOfWeekIsMon) {
    ...
    const dates = Day(this.vm).buildDate(year, month);
...
export default vm => new WeekMode(vm);
