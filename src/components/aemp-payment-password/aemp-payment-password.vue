<template>
	<view class="best-modal digital-keyboard-modal" :class="{'best-modal-active':_show}">
		<view class="best-modal-layer" @tap="cancel">取消交易</view>
		<view class="best-modal-content">
			...
			<view class="pwd-box clearfix" @tap="getKeyboard">
				<view class="pwd-text" v-for="(item,index) in _digits" :key="index"
			...
			<view class="digital-keyboard">
				<view class="form_edit clearfix">
					<view class="num" v-for="item in digitalList" :key="item"
						:class="{'no-num':(item === '' || item === '-1')}" @tap="getKeyNumber(item)">{{item}}</view>
				</view>
			</view>
		</view>
	</view>
...
<script>
	export default {
		...
		data() {
			return {
				activeInput: 0,
				digitalList: ['2', '1', '3', '4', '5', '6', '7', '8', '9', '', '0', '-1'],
				paymentPwd: ''
			};
		},
		props: {
